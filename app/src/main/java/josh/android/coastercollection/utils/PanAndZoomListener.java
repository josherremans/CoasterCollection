...
public class PanAndZoomListener implements OnTouchListener {
	...
	private static final String LOG_TAG = "PanAndZoomListener";
...
//	private Log log;
	...
	public PanAndZoomListener(Context context, FrameLayout container, View view, int anchor) {
		cx = context;
		
//		log = Log.getInstance(cx);
...
//		log.writeInfo(LOG_TAG + ": event: " + (event.getAction() & MotionEvent.ACTION_MASK));

		switch (event.getAction() & MotionEvent.ACTION_MASK) {
		
		case MotionEvent.ACTION_DOWN:
//			log.writeInfo(LOG_TAG + ": mode: DRAG");
			
			mode = DRAG;
			
			child.getParent().requestDisallowInterceptTouchEvent(true); // JOHER
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			...
			if (oldDist > 10f) {
//				log.writeInfo(LOG_TAG + ": mode: ZOOM");
...
//			log.writeInfo(LOG_TAG + ": mode: NONE");
...
//				log.writeInfo(LOG_TAG + ": newDist: " + newDist);
				...
				if (newDist > 10f) {
					...
					currentZoom = panZoomCalculator.doZoom(scale, mid);
				}
			}
		...
		case MotionEvent.ACTION_CANCEL:			
			view.getParent().requestDisallowInterceptTouchEvent(true); // JOHER
...
//		View child; // JOHER
		...
		PanZoomCalculator(View container, View childView, int anchor) {
		...
		public float doZoom(float scale, PointF zoomCenter) {
...
//						log.writeInfo(LOG_TAG + ": fitToWindow=" + fitToWindow);
//						log.writeInfo(LOG_TAG + ": currentZoom=" + currentZoom);
//						log.writeInfo(LOG_TAG + ": winWidth=" + winWidth + ", bmWidth=" + bmWidth);
//						
//						log.writeInfo(LOG_TAG + ": currentPan.x=" + currentPan.x + ", xOffset=" + xOffset);
						
						// JOHER
						// TODO: Nog toevoegen indien in landscape mode!
						if ((currentPan.x == 0.0f)
								|| (((currentPan.x - winWidth)) - (-1.0f * winWidth * currentZoom)) < 0.01f ) {
//							log.writeInfo(LOG_TAG + ": PARENT HAS CONTROL");
							
							if (child != null) {							
								child.getParent().requestDisallowInterceptTouchEvent(false);
							}
						} else {
//							log.writeInfo(LOG_TAG + ": CLIENT HAS CONTROL");
